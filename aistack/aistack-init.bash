...
# - NVidia CUDA v.9.x (recommended: 9.2.x) or newer;
# - NVidia CUDNN v.7.x (recommended: 7.4.x) or newer;
# - NVidia NCCL (whichever version compatible with CUDA; recommended: 2.x);
...
# SKLearn-Bayes
git clone --recursive https://github.com/AmazaspShumik/sklearn-bayes.git
cd sklearn-bayes
pip install --upgrade --no-deps .
cd ../
...
git clone --recursive https://github.com/creafz/pytorch-cnn-finetune.git
cd pytorch-cnn-finetune
pip install --upgrade --no-deps .
cd ../
...
# DataCleaner
pip install --upgrade --no-deps datacleaner
