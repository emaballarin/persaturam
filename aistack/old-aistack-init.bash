#!/bin/bash
#########################################
## AIStack, v. 0.6.5-EOL (15/09/2018)  ##
##        End Of Life version          ##
#########################################
#
...
export SELF_TENSORCOMP="0"                                              # Set to "1" to install Tensor Comprehensions and make it work together with PyTorch
                                                                        # NOTE: Tensor Comprehensions requires PyTorch v.0.3 OR PREVIOUS. For now, you are recommended not to use it.
...
if [ "$SELF_FIRSTRUN" = "1" ]; then
    ...
    wget --tries=0 --retry-connrefused --continue --progress=bar --show-progress --timeout=30 --dns-timeout=30 --random-wait https://gist.githubusercontent.com/emaballarin/7cbf5ace151cb1b53ce9a57a483aeae3/raw/6a018b428fba4176ff9306956cff1466b25a59bd/dot-condarc
...
# Update Conda environment (regardless of first run or not)
source $SELF_CEACT_COMMAND $SELF_CONDA_ENV_NAME
conda upgrade -y --all
conda remove -y cmake cudatoolkit curl --force
source deactivate
...
conda install -y --force matplotlib scipy networkx
conda install -y --force websocket-client docker-pycreds -c conda-forge
conda upgrade -y --all
conda upgrade -y --all
conda remove -y cmake cudatoolkit curl --force
